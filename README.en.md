...
| [TESTCLIENT](TESTCLIENT/README/README.md) | terminal test client: hardware tests, sharded inference, evaluation | Phase 1 complete, plus items 2.1 and 3.1. `vergleich` compares the logs of several machines and issues the verdict, and **refuses** one when every log comes from the same machine. Plus a menu entry for free inference. The proof itself still awaits heterogeneous hardware |
