...
| [TESTCLIENT](TESTCLIENT/README/README.md) | terminal test client: hardware tests, sharded inference, evaluation | Phase 1 complete, plus items 2.1 and 3.1. `vergleich` compares the logs of several machines and issues the verdict — and **refuses** one when every log comes from the same machine. Received logs go in `TESTCLIENT/Vergleiche/`, the detailed report into its `Berichte/` subfolder. Every run records the model state (θ_v, anchor digest); if it differs, the verdict reads "not comparable", not "hardware finding". One log per test run, named after participant and settings. Arrow-key navigation with a line-based fallback. Test plans carry several prompts; two ship with the client (0.5B and 7B). All launchers live in `TESTCLIENT/` and keep working when moved. The proof itself still awaits heterogeneous hardware |
