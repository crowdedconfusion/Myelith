...
| [TESTCLIENT](TESTCLIENT/README/README.md) | terminal test client: hardware tests, sharded inference | Phase 1 complete. All launchers live in `TESTCLIENT/`: an app bundle for macOS, a `.cmd` for Windows, a shell script for the terminal. They locate the repository root themselves and keep working when moved. It finds artifacts on its own, offers a choice when several exist, and otherwise builds them from HF weights plus the scale pack. It verifies the digest and states explicitly that a mismatch is **not** a hardware finding. Phase 2 awaits heterogeneous hardware |
