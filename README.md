...
| [TESTCLIENT](TESTCLIENT/README/README.md) | Terminal-Testclient: Hardwaretests, geshardete Inferenz, Auswertung | Phase 1 abgeschlossen, dazu Punkt 2.1 und 3.1. `vergleich` stellt die Protokolle mehrerer Maschinen gegenüber und fällt das Urteil — und **verweigert** es, wenn alle von derselben Maschine stammen. Die zugesandten Protokolle liegen in `TESTCLIENT/Vergleiche/`, der ausführliche Bericht in dessen Unterordner `Berichte/`. Jeder Lauf protokolliert den Modellstand (θ_v, Ankerdigest); weicht er ab, lautet das Urteil ausdrücklich „unvergleichbar", nicht „Hardware-Befund". Ein Protokoll je Testlauf, benannt nach Teilnehmer und Einstellungen. Bedienung über Pfeiltasten mit zeilenweisem Rückfall. Testpläne tragen mehrere Prompts; zwei liegen bei (0,5B und 7B). Alle Starter liegen in `TESTCLIENT/` und funktionieren auch verschoben. Der eigentliche Nachweis wartet weiter auf heterogene Hardware |
