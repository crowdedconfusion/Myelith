...
| [NETWORKING](NETWORKING/README/README.md) | P2P-Gossip, Peer-Discovery, Latenztopologie | Phase 2 abgeschlossen: Paarlatenzmessung, LatencyGraph, Geo- und AS-Diversität. Seit v0.3.0 eine **adversariale Testebene für die Gossip-Parser**, und sie fand zweierlei: Die Latenz-EMA rechnete in Gleitkomma, obwohl der Kopf des Crates Festkomma zusagt, und der Gleitkomma-Audit konnte es nicht finden, weil `myl-net` mit **keiner einzigen Datei** in seiner Liste stand (Fund 44, acht Dateien nachgetragen). Dazu die Messung, wie viel die Strukturprüfung wirklich filtert: Bei Typen aus lauter Feldern fester Länge ist ein Borsh-Parse **eine Längenprüfung**, und 20 000 von 20 000 verstümmelten PoI-Bündeln kommen durch (Fund 45) Seit **v0.4.0** eine Verbindungsgrenze (Fund 53): eingehende und ausgehende Verbindungen bekommen **getrennte Budgets**, sodass eine Flut die selbst gewählten Plätze nicht aufzehren kann. Die Zusage lautet „der Knoten darf wählen", nicht „er wählt richtig" — wer die Bootstrap-Liste stellt, umgeht sie. Zwei Funde dabei: eine Härtung, die den ehrlichen Knoten mit stummschaltete (Fund 54), und ein dokumentierter Prüf-Einstieg, der über die Laufzeit nie erreichbar war (Fund 55). **v0.5.0** bringt die NAT-Überwindung (AutoNAT, Relais, DCUtR, QUIC): Vorher konnten nur öffentlich erreichbare Knoten mitmachen, und das ist nicht nur unbequem, sondern treibt β aus der Kollusionsrechnung nach oben. Belegt an einem Knoten, der auf keiner wählbaren Adresse horcht und trotzdem erreicht wird; **das Lochstanzen selbst bleibt ungeprüft**, es braucht zwei echte NATs (Fund 56: ein Relais ohne eigene Adresse nimmt Reservierungen an und antwortet ins Leere) |
| [NODE](NODE/README/README.md) | Der Knoten: das Programm, das das Protokoll ausführt | **Neu am 2026-08-24.** Bis dahin hatte das Projekt dreizehn Komponenten, rund 1500 Tests und **kein Programm, das einen Knoten startet**; `myl-net` hatte im ganzen Repositorium keinen einzigen Abnehmer. Das war die Ursache einer ganzen Fundklasse: Die Funde 52, 55, 56 und 57 wurden alle sichtbar, als jemand die Teile zusammensteckte. Der Knoten läuft über TCP und QUIC, arbeitet hinter NAT über Relais und schreibt ein JSONL-Betriebsprotokoll, das `myl-test netz` auswertet. Belegt an zwei echten Prozessen. **Er produziert keine Blöcke:** Die Zustandsmaschinen sind vollständig, aber niemand treibt sie über die Zeit, es fehlen Rundentakt, Mempool und Kettenzustand. ⚑ Fund 57 dabei: Auch für Blöcke ist der Borsh-Parse fast nur eine Längenprüfung, rund 88 % der verstümmelten kommen durch |
...
| [COMPUTE_PIPELINE](COMPUTE_PIPELINE/README/README.md) | Pod-Orchestrierung über echtes Netz | Phase 2.1 abgeschlossen: Micro-Batching und Pipelining. **Pipeline-Determinismus wieder bitgleich mit dem Einzelknoten,** seit der verlustbehaftete Boundary-Schritt zwischen den Stages entfallen ist (Funde 20/26). Die Spur bindet damit wieder die übertragene Aktivierung. Seit v0.3.0 ist „bitgleich" hier über die **gerechneten Zahlen** belegt und nicht mehr über die erzeugten Token: Der Pod gibt einen Digest über Logits und Token heraus, und er stimmt bei 1 bis 24 Shards mit dem Einzelknoten überein (Fund 36). Seit v0.6.0 gibt es eine **adversariale Testebene für das Drahtformat**, und sie fand beim ersten Lauf einen Fehler: Die Manipulationserkennung ging bei leerer Spur durch, und eine unpassende Länge ließ den Kernel abstürzen (Fund 41, behoben). ⚑ **Seit v0.9.0 ist der Vergütungspfad zum ersten Mal durchgängig.** Der Test des byzantinischen Koordinators steckte Pod und Konsens zum ersten Mal zusammen und fand, dass sie nicht zusammenpassten: Das Bündel des Pods war über die **Übergangs-Signaturen** aggregiert, der Konsens prüfte gegen die **Bündelbotschaft**, und damit verifizierte kein Bündel je (Fund 52). Es fehlte kein Code, sondern ein Protokollschritt. Jetzt sehen die Mitglieder das **fertige** Bündel, prüfen den beanspruchten Arbeitsumfang gegen ihre eigene Segmentzahl und unterschreiben erst dann; eine Unterschrift ohne Prüfung wäre keine Zustimmung, sondern eine Anwesenheitsnotiz. **Seit v0.7.0 steht die Ausfallsicherung:** Standby-Übernahme und KV-Cache-Rebuild, und die Liveness-Zusage aus Kap. 6.8 ist **gemessen statt zugesagt** — beide Hälften, zwei Ausfälle ja und drei nein. Dass der Standby seinen Cache **bitgleich** nachbauen kann, ist dabei keine Selbstverständlichkeit, sondern eine Folge der Grundentscheidung: Ein Prefill über dieselben Token liefert ganzzahlig denselben Cache; in einem Gleitkomma-System wäre der Rebuild ein Bruch der Sitzung. Seit v0.5.0 trägt die **Spur einen Eintrag je Layer statt je Shard**: Ihre Länge hängt damit am Modell und nicht mehr am Zuschnitt, zwei Pods mit verschiedener Knotenzahl sind vergleichbar, und die Bisektion grenzt die fehlerhafte Layer ein statt der Layer-Gruppe |
...
Angriffsklassen aus Whitepaper Kap. 5.6 und 9.2 auf: **acht sind
abgewehrt und belegt, drei offen mit gemessener Lücke, zwei ungeprüft.**
Es ist kein externes Review und ersetzt keines.

Die drei offenen sind benannt statt umschrieben: Ein Knoten **nimmt
beliebig viele Verbindungen an** (keine Verbindungsgrenze, kein
Peer-Scoring), die **Signatur der Latenz-Atteste prüft niemand**, und die
**Ununterscheidbarkeit der Kontrollsegmente** ist eine offene Messfrage,
die das Whitepaper selbst so führt. Alle drei hängen an derselben Stelle:
Wer steuert, was ein Knoten beobachtet, steuert die Verifikation.

Ein eigenes Crate [`myl-simulation`](SIMULATION/) fährt den Weg eines
Segments durch alle Schichten und prüft damit die **Nähte**. Der Grund
steht in der Fundhistorie: Fast jeder schwere Fund dieses Projekts saß
nicht in einer Komponente, sondern zwischen zweien, und jeder war in
seiner Komponente korrekt.
