...
Relative to v0.1, v0.2 refines verification at one point that has proved consequential: the bit-equality requirement is a prescription about the *order* of operations, not about their *precision*. This removes any reason to mandate a particular accumulation format. Which would disadvantage consumer hardware relative to data centers and run counter to decentralization. Chapter 6.3 documents why the floating-point alternatives, fixed ordering, tolerance comparison, and software emulation, do not hold up under examination. Relative to v0.2, Chapter 6.9 is added, recording first findings from the reference implementation: bit width must be fixed per tensor, and the execution specification must be obtained from the weights rather than the architecture description.
...
At p = 2% and g = one segment's reward, $S_{\min} = 2500$ segment rewards, the stake thus corresponds to roughly twelve epochs of income. That is the quantitative anchor for the stake parameters and is adjusted via governance to measured fraud rates.
...
**Early-phase concentration.** Those who participate early acquire a disproportionate share of what is minted overall. With work-bound issuance this is unavoidable and is documented for fair launches: even without pre-sale and insider allocation, early miners can amass considerable holdings [38]. The remedy is not a different distribution but a flat subsidy curve: the lower the initial subsidy s, the smaller the advantage of early participation. The parameter is subject to governance (Ch. 10.3) and to the invariant of Appendix B.4.
---
...
### 6.9 Interim Findings from the Reference Implementation

Since version 0.2 of this paper, a reference implementation has been under development that tests the assumptions of 6.2 against a real model. Its state is incomplete: bit-exact execution across multiple hardware classes is still outstanding, as is the measurement of output quality. Three findings are nonetheless already firm and are recorded here because they bear on the execution specification.

**The specification is complete only at the real model.** Several assumptions plausibly derived from the model architecture proved incorrect once actual weights were available: the presence of bias terms in the attention projections, the arrangement of key and value heads under grouped-query attention, and the value range actually occurring at the input of the exponential function. None of these concerns determinism as such, but each would have produced wrong results. For Chapter 10.1 a tightening follows: a model's execution specification cannot be derived from its architecture description, only from its weights. It is therefore a measurement result, not a design document.

**Not every tensor tolerates the same bit width.** The most serious finding concerns models that carry input embedding and output projection on the same weight. The error tolerance of the two uses differs fundamentally: in the embedding lookup the quantization error acts additively on a residual stream of far greater amplitude and remains inconsequential. In the output projection the same error directly decides the ranking of tokens. For a model with 0.5 billion parameters the induced noise exceeds the spread of the leading logits, so that selection becomes effectively random.

An addition to the stipulations of 6.2 follows: bit width is to be fixed **per tensor** and forms part of θ_v, not globally for the entire model. Determinism is unaffected, since a higher bit width does not alter associativity; only memory requirements are concerned. Which bit width each tensor requires is a measurement question and model-dependent (Ch. 11, point 1).

**Small models are the least favorable case.** The preceding finding comes from a model at the lower end of the size range. As model size grows, logit spreads widen, so that a given quantization error weighs relatively less. A result obtained on a small model therefore permits no inference about the scale intended in this paper, in either direction.
...
**Cost.** A growth step requires roughly one third of the tokens a pre-training run of the same size would cost, since prior knowledge is preserved (Appendix B.6). The literature reports savings between forty and nearly seventy percent for progressive growth [27][28][29].
...
**The combination of methods is unproven.** Integer training is established [23][24], model growth is established [25]–[29], their combination is not. Moreover, the evidence for integer training comes from the image domain with comparatively small networks; for transformers at the scale intended here no demonstration exists.
...
1. **Output quality of integer inference at the target scale (M0).** *Partial result available (Ch. 6.9):* bit width must be fixed per tensor; a globally uniform scheme does not suffice where embedding and output projection share the same weight. The measurement proper remains open. The verification of Chapter 6 presupposes a model executable entirely in integer arithmetic. Eight-bit quantization is broadly established for transformers [18][19]; its extension to large language models is more recent [20] and not comprehensively validated at the intended scale. To be measured: the quality gap to the floating-point reference on established benchmarks. Should it prove too large, the basis of the chapter requires reassessment; the fallback would be tolerance-based commitments (point 10).
2. **Completeness of the execution specification (M0).** *Partial result available (Ch. 6.9):* several assumptions derived from the architecture description proved incorrect at the real model. The specification must be obtained from the weights, not from the documentation. The determinism property rests on all platform-dependent operations being covered. To be examined in particular: the behavior of integer matrix units at range boundaries, saturation semantics, possible compiler transformations, and the approximations of the non-linear functions. The result is a conformance suite with test vectors that admits new hardware without protocol changes.
...
9. **Integer training for language models (M3).** The evidence for integer training comes from the image domain with comparatively small networks [23][24]. To be measured: the quality gap to floating-point training for transformers at the target scale, and the gap between local loss blocks and global backpropagation (Ch. 7.2).
10. **Combining integer training with model growth (M3/M4).** Both methods are established individually, their combination is not. To be examined in particular: whether function-preserving expansion remains exact under integer representation or introduces rounding error (Ch. 7.5).
...
async fn shard_loop(shard: ShardWeights, role: PodRole) {
    loop {
        ...
        // 3. Forward pass according to theta_v (prescribed reduction order)
...
State transitions (myl-ledger):
  burn(addr, myl)            → mint_credits(addr, myl / price_e)
  settle_epoch(e)            → compute M_e (EMA burn), distribute rewards
...
**Numerical example:** p = 0.02, segment reward g = 0.5 MYL yields $S_{\min} = 1250$ MYL per segment of capacity. A miner with a capacity of 100 segments/epoch needs stake of ~1250 segment rewards ≈ 12.5 epochs of income. With one-hour epochs: roughly half a day's income as a pledge per unit of capacity, practicable, and tightenable via p if fraud cases are observed.
...
**B.6.11 Limits of these analyses.** All values stem from model calculations in software, not from training runs. The capacity calculation rests on an efficiency assumption of 25 percent of peak performance over WAN; the growth calculation on a conservatively assumed fifty percent saving relative to pre-training. The models for forgetting and benchmark manipulation are qualitative and serve orders of magnitude, not prediction.
...
The model calculations of this appendix are included as executable programs in the project repository under `py/simulations/`. They require no dependencies beyond the standard library.
...
5. Gensyn: Litepaper: Verifiable Deep Learning Compute Protocol. Technical report. https://www.gensyn.ai/
...
38. Fair Launch Token Distribution: Mechanisms, Benefits and Limitations. Survey, 2025.
