...
**Grenzen dieses Ergebnisses.** Gemessen wurde an einem Modell am unteren Ende der Größenskala, das für Quantisierung den ungünstigsten Fall darstellt: Mit wachsender Modellgröße nehmen die Abstände zwischen den führenden Logits zu, sodass ein gegebener Fehler relativ weniger wiegt. Für die im Papier angestrebte Größenordnung ist damit ein günstigeres Ergebnis zu erwarten, aber nicht belegt. Ebenso steht der Nachweis aus, dass die Bitgleichheit über verschiedene Hardware-Klassen hinweg gilt; die vorliegenden Läufe erfolgten auf einer Referenzimplementierung ohne Beschleuniger (Kap. 11, Punkte 2 und 3).
...
2. **Vollständigkeit der Ausführungsspezifikation (M0).** *Teilergebnis liegt vor (Kap. 6.9):* Die Spezifikation ist aus den Gewichten zu gewinnen, nicht aus der Dokumentation; auf einer Referenzimplementierung ohne Beschleuniger ist sie vollständig. Offen bleibt der Nachweis über verschiedene Hardware-Klassen hinweg. Die Determinismus-Eigenschaft beruht darauf, dass alle plattformabhängigen Operationen erfasst sind. Zu prüfen sind insbesondere das Verhalten ganzzahliger Matrixeinheiten an den Bereichsgrenzen, die Sättigungssemantik, mögliche Compiler-Umformungen sowie die Approximationen der nichtlinearen Funktionen. Ergebnis ist eine Konformitätssuite mit Testvektoren, die neue Hardware ohne Protokolländerung aufnimmt.
