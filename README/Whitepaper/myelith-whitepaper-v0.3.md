...
Die Annahme aus 6.2, dass sich ein Sprachmodell vollständig ganzzahlig ausführen lässt, ohne die Ausgabequalität nennenswert einzubüßen, wurde seit Version 0.2 dieses Papiers an einer Referenzimplementierung geprüft. Grundlage ist ein dense Modell mit 0,5 Milliarden Parametern unter permissiver Lizenz; Gewichte in acht Bit mit kanalweisen Zweierpotenz-Skalen, Aktivierungen in sechzehn Bit mit kalibrierten Skalen je Schicht.

**Ergebnis.** Die Perplexität auf einem festen Auszug aus WikiText-2 beträgt 15,59 gegenüber 14,95 in der Gleitkomma-Referenz, gemessen mit identischer Methode auf denselben Sequenzen. Der relative Abstand liegt bei 4,3 Prozent. Die Übereinstimmung der jeweils wahrscheinlichsten Token mit der Gleitkomma-Referenz beträgt 89,3 Prozent.
...
**Der Nachweis erstreckt sich auch auf verteilte Ausführung.** Eine vierstufige Pipeline aus tatsächlich getrennten Betriebssystem-Prozessen, die über echte TCP-Verbindungen kommunizieren, erzeugt bei wiederholtem identischem Prompt dieselbe Token-Sequenz wie die Einzelknoten-Referenz und ist über unabhängige Läufe deterministisch. Unter künstlich injizierter Netzwerklast — Verzögerung von 100 Millisekunden je Stage-Übergang, Verbindungsabbrüche mit Retry-Logik über Duplikaterkennung, vollständiger Neustart einzelner Knoten mit Idempotenz-Prüfung — bleibt das Ergebnis bitgleich. Damit ist die Annahme aus 6.2, dass unabhängig ausführende Knoten zum selben Ergebnis kommen, erstmals nicht nur innerhalb eines Prozesses, sondern über das Netzwerkprotokoll hinweg zwischen getrennten Prozessen geprüft — bislang allerdings auf einer einzelnen Maschine (Loopback-Verbindungen), nicht über räumlich getrennte Hosts.

**Wo die Kosten entstehen — und wo nicht.** Aufschlussreicher als die Endzahl ist die Zerlegung des Fehlers. Führt man dieselbe Quantisierung in Gleitkomma-Arithmetik aus, ergibt sich:

| Konfiguration | Perplexität | Abstand |
|---|---|---|
| Gleitkomma, nicht quantisiert | 14,95 | Referenz |
| Gewichte in acht Bit, Arithmetik in Gleitkomma | 15,05 | +0,7 % |
| Gewichte in sechzehn Bit, Arithmetik in Gleitkomma | 14,94 | −0,1 % |
| Vollständig ganzzahliger Pfad | 15,59 | +4,3 % |

Die Gewichtsquantisierung kostet also fast nichts, und eine Verdopplung der Bitbreite bringt keinen Gewinn. Der gesamte Abstand entsteht in den Zwischenstufen des ganzzahligen Pfades: den Quantisierungen zwischen den Teilschritten, den ganzzahligen Approximationen der nichtlinearen Funktionen und der Rundung bei jeder Reskalierung.

Für die Praxis folgt daraus eine klare Priorität: Wer ganzzahlige Ausführung umsetzt, sollte den Aufwand nicht in die Gewichtsdarstellung investieren, sondern in die Genauigkeit der Zwischenberechnungen. Die vorliegende Implementierung erreichte das genannte Ergebnis erst, nachdem mehrere naheliegende Wege — feinere Gewichtsquantisierung nach dem Verfahren von [44], Erhöhung der Gewichtsbitbreite, Verbreiterung des Kalibrierungskorpus — nachweislich ohne Wirkung geblieben waren.
...
**Grenzen dieses Ergebnisses.** Gemessen wurde an einem Modell am unteren Ende der Größenskala, das für Quantisierung den ungünstigsten Fall darstellt: Mit wachsender Modellgröße nehmen die Abstände zwischen den führenden Logits zu, sodass ein gegebener Fehler relativ weniger wiegt. Für die im Papier angestrebte Größenordnung ist damit ein günstigeres Ergebnis zu erwarten, aber nicht belegt. Ebenso steht der Nachweis aus, dass die Bitgleichheit über verschiedene Hardware-Klassen und über räumlich getrennte Hosts hinweg gilt; die vorliegenden Läufe erfolgten auf einer Referenzimplementierung ohne Beschleuniger, die Mehrknoten-Läufe bislang auf einer einzelnen Maschine (Kap. 11, Punkte 2 und 3).
...
1. **Ausgabequalität ganzzahliger Inferenz in der Zielgrößenordnung (M0).** *Gemessen für 0,5 Milliarden Parameter (Kap. 6.9): 4,3 Prozent Abstand zur Gleitkomma-Referenz.* Offen bleibt die Übertragung auf die angestrebte Größenordnung, für die ein günstigeres Ergebnis zu erwarten, aber nicht belegt ist. Die Verifikation aus Kapitel 6 setzt ein vollständig ganzzahlig ausführbares Modell voraus. Acht-Bit-Quantisierung ist für Transformer breit belegt [18][19], die Übertragung auf große Sprachmodelle ist jüngeren Datums [20] und für die angestrebte Größenordnung nicht umfassend validiert. Zu messen ist der Qualitätsabstand zur Gleitkomma-Referenz auf etablierten Benchmarks. Fällt er zu groß aus, ist die Grundlage des Kapitels neu zu bewerten; die Rückfalloption wären toleranzbasierte Commitments (Punkt 10).
2. **Vollständigkeit der Ausführungsspezifikation (M0).** *Teilergebnis liegt vor (Kap. 6.9):* Die Spezifikation ist aus den Gewichten zu gewinnen, nicht aus der Dokumentation; auf einer Referenzimplementierung ohne Beschleuniger ist sie vollständig, auch über getrennte, netzwerkgekoppelte Prozesse hinweg. Offen bleibt der Nachweis über verschiedene Hardware-Klassen und über räumlich getrennte Hosts hinweg. Die Determinismus-Eigenschaft beruht darauf, dass alle plattformabhängigen Operationen erfasst sind. Zu prüfen sind insbesondere das Verhalten ganzzahliger Matrixeinheiten an den Bereichsgrenzen, die Sättigungssemantik, mögliche Compiler-Umformungen sowie die Approximationen der nichtlinearen Funktionen. Ergebnis ist eine Konformitätssuite mit Testvektoren, die neue Hardware ohne Protokolländerung aufnimmt.
...
Die Datentypen aus A.1 und die Algorithmen aus A.3 und A.5 sind inzwischen nicht mehr nur Referenzalgorithmen, sondern haben eine getestete Implementierung: `myl-types` (Merkle-Baum, VRF nach RFC 9381, BLS12-381-Signaturen, die Structs aus A.1 mit exakter Feldreihenfolge als Konsens-Vertrag), `myl-pod` (der Mining-Loop aus A.3 einschließlich Manipulationserkennung) und `myl-ledger` (die Zustandsübergänge aus A.5, deren Determinismus über unabhängige Läufe geprüft ist). Der aktuelle Implementierungsstand ist im Repository dokumentiert, nicht in diesem Papier.
