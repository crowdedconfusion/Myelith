...
# 📌 **Gezaehlt wird, wo Gewichte liegen, nicht wo Ordner stehen**
# (Fund 471, 2026-09-25). Jeder Modellordner ist mit seiner LICENSE
# versioniert; die alte Zaehlung meldete deshalb in jedem frischen Klon
# „Gewichte da: 5", ohne dass eine einzige Gewichtsdatei lag.
GEWICHTE=$(find "$WURZEL/MODELS/llm" -maxdepth 2 -name '*.safetensors' 2>/dev/null | sed 's|/[^/]*$||' | sort -u | wc -l | tr -d ' ')
